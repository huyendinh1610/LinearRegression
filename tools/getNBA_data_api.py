...
def get_df_with_api(season_str):
    season_url = url.format(season=season_str)
    response = r.get(season_url, headers=headers, timeout=5)

    data = response.json()
    resultsets = data["resultSets"][0]

    cols = resultsets["headers"]
    rowSet = resultsets["rowSet"]
    #print(rowSet)
    df = pd.DataFrame(rowSet, columns=cols)
